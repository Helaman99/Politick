    ...
    <div class="main-content">
      <div class="side" id="left">
        ...
        <div class="pack-buttons">
          <v-btn
            v-for="wordPack in selectedPackList"
            v-bind:key="wordPack"
            @click="selectedPack = wordPack"
            :disabled="
              player?.unlockedTitleFirstWords.includes(wordPack.contents[0]) ||
      ...
      <v-dialog class="prompt" v-model="buyPrompt">
        <v-card class="prompt-card">
          ...
          <div class="prompt-right">
            <p>
              Are you sure you would like to buy the "{{ selectedPack.name }}" word pack for
              {{ selectedPack.price }} coins?
            </p>
            <v-card-actions>
