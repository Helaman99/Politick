export class Player {
  ...
  get standingActual() {
    ...
    if (x > 1.3 || (this._left != 0 && this._right == 0)) standing += 'Left '
    else if (x < 0.7 || (this._right != 0 && this._left == 0)) standing += 'Right '
    else standing = 'Center '
    if (y > 1.3 || (this._authoritarian != 0 && this._libertarian == 0)) standing += 'Authoritarian'
    else if (y < 0.7 || (this._libertarian != 0 && this._authoritarian == 0))
      standing += 'Libertarian'
    else if (standing != 'Center ') standing = 'Center ' + standing
  ...
  addTitleFirstWords(newWords: string[]) {
    if (newWords.length != 0)
      for (let i = 0; i < newWords.length; i++) this._unlockedTitleFirstWords.push(newWords[i])
  }

  addTitleSecondWords(newWords: string[]) {
    if (newWords.length != 0)
      for (let i = 0; i < newWords.length; i++) this._unlockedTitleSecondWords.push(newWords[i])
  }

  addAvatar(newAvatar: string) {
    if (newAvatar != null && newAvatar != '') this._unlockedAvatars.push(newAvatar)
  }
  ...
  set avatar(newAvatar: string) {
    if (newAvatar != null && newAvatar != '') this._avatar = newAvatar
  }

  set title(newTitle: string) {
    if (newTitle != null && newTitle != '') this._title = newTitle
  }
